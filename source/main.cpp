...
namespace CTRPluginFramework
{
	...
	Process::ExceptionCallbackState exceptHdl(ERRF_ExceptionInfo *excep, CpuRegisters *regs)
	{
		OSD::Lock();
		Screen bot = OSD::GetBottomScreen();
		bot.DrawRect(0, 0, 320, 240, Color());
		bot.Draw("[Insert error info here]", 8, 8);
	...
	int main(void)
	{
		...
		Process::exceptionCallback = exceptHdl;
